...
# Use a more stable Python version (3.12 instead of 3.13 alpha)
FROM python:3.12-alpine AS base

# Prevents Python from writing pyc files and ensures logs are flushed
ENV PYTHONDONTWRITEBYTECODE=1
ENV PYTHONUNBUFFERED=1

WORKDIR /app

# Upgrade Alpine packages and fix expat vulnerabilities
RUN apk update && apk upgrade --no-cache expat
